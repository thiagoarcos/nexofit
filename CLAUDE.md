...
- Gym → **Rutina** and Gym → **Programa** are the *same* underlying data (`state.program`, a variable-length array of weeks → days → exercises with per-set weight/reps/RIR and auto tonnage via `tonnage()`), rendered by one shared function `ProgramaView(mode)` (`mode: "rutina" | "programa"`) — don't reintroduce a second data model for one of them. `state.currentWeek`/`state.currentDay` is a single persisted pointer shared by both tabs and by Hoy's "today's workout" card (manually advanced by the user — the program isn't tied to calendar weekdays). `mode: "rutina"` additionally shows the done-checkbox (`toggleEx`, logged into `workoutLog`/`sessionLog`/`exerciseHistory`), PR badge, and an ℹ️ technique/video panel that looks up `EXDB` by exact exercise name match. Programa's week/day counts are dynamic (not hardcoded to 5/7), since importing a `.xlsx` routine can produce any shape — always guard `state.program.weeks[state.currentWeek]` / `.days[state.currentDay]` for `undefined` before rendering editable UI.
